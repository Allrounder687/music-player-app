...
async function fetchSongsFromSource(source) {
  try {
    ...
    // Normalize song format and enrich with MusicBrainz data if possible
    const normalizedSongs = await Promise.all(limitedSongs.map(async (song, index) => {
      const id = 'online_' + source.name.toLowerCase().replace(/\\s+/g, '_') + '_' + index + '_' + Date.now();

      let title, artist, album, previewUrl, imageUrl, year, genre, songDuration;
      ...
      if (song.title && (song.streamUrl || song.url || song.previewUrl || song.src)) {
        ...
        year = song.year;
        genre = song.genre;
        imageUrl = song.imageUrl || song.albumArt || song.thumbnail;
        songDuration = song.duration;

        // Try to enrich with metadata APIs for sample songs
        if (song.canEnrich && source.name === 'Sample Songs API') {
          try {
            const { MetadataService } = await import('../services/metadataService.js');

            // Create a track object with language detection
            const trackForEnrichment = {
              artist,
              title,
              language: song.language || (song.genre === 'Bollywood' ? 'hindi' : 'english')
            };

            const enrichedData = await MetadataService.enrichMetadata(trackForEnrichment);

            if (enrichedData.imageUrl) {
              imageUrl = enrichedData.imageUrl;
              const apiSource = enrichedData.source || 'API';
              console.log(`Enriched ${title} by ${artist} with ${apiSource} album art`);
            }

            // Update other metadata if available
            if (enrichedData.album) album = enrichedData.album;
            if (enrichedData.year) year = enrichedData.year;
            if (enrichedData.genre) genre = enrichedData.genre;
            if (enrichedData.duration) songDuration = enrichedData.duration;
          } catch (enrichError) {
            console.log(`Could not enrich ${title} by ${artist}:`, enrichError.message);
          }
        }
      } else {
      ...
      return {
        ...
        year,
        genre,
        imageUrl: imageUrl || '/images/album-placeholder.svg',
        previewUrl,
        source: 'Online',
        sourceName: source.name,
        isOnline: true
