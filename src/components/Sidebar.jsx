...
export const Sidebar = () => {
  ...
  const navItems = [
    ...
    {
      to: "/playlists",
      icon: <FaList className="mr-3" />,
      label: "Playlists",
      count: Object.keys(playlists.custom || {}).length,
    },
  ];
      ...
      >
        <div className="p-4 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <FaMusic
              className={`text-${theme.colors.primary.main} text-xl animate-pulse`}
            />
        ...
        <nav className="flex-1 p-2 space-y-1">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.count !== null && (
                <span
                  className={`text-xs px-2 py-1 rounded-full bg-${theme.colors.background.main} text-${theme.colors.text.muted} group-hover:bg-opacity-80 transition-all`}
                >
                  {item.count}
                </span>
              )}
        ...
        {showKeyboardShortcuts && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div
              className={`bg-${theme.colors.background.main} rounded-lg p-6 m-4 max-w-md w-full max-h-96 overflow-y-auto`}
            >
              <div className="flex items-center justify-between mb-4">
                <h3
                  className={`text-xl font-bold text-${theme.colors.text.primary}`}
                >
                  Keyboard Shortcuts
                </h3>
                <button
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <kbd
                      className={`px-2 py-1 bg-${theme.colors.background.secondary} rounded text-sm`}
                    >
                      Space
                    </kbd>
