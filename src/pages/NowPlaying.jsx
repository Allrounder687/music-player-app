...
export const NowPlaying = () => {
  ...
  if (!currentTrack) {
    return (
      <div
        className={`p-6 h-full overflow-y-auto bg-${theme.colors.background.main}`}
      >
        <div className="max-w-6xl mx-auto">
          {/* Welcome Header */}
          <div className="text-center mb-12">
            <div
              className={`w-24 h-24 mx-auto mb-6 bg-${theme.colors.primary.main} rounded-full flex items-center justify-center`}
            >
              <FaMusic className="text-4xl text-white" />
            </div>
            <h1
              className={`text-4xl font-bold text-${theme.colors.text.primary} mb-4`}
            >
              Welcome to Your Music
            </h1>
            <p
              className={`text-lg text-${theme.colors.text.muted} max-w-2xl mx-auto`}
            >
              Discover, organize, and enjoy your music collection with our
              modern player. Start by selecting a track from your library.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div
              className={`bg-${theme.colors.background.secondary} rounded-lg p-6 text-center`}
            >
              <h3
                className={`text-2xl font-bold text-${theme.colors.primary.main} mb-2`}
              >
                {tracks.length}
              </h3>
          ...
          <div className="mb-8">
            <h2
              className={`text-2xl font-bold text-${theme.colors.text.primary} mb-6`}
            >
                ...
                >
                  <div className="relative">
                    {track.imageUrl ? (
                      ...
                      <div
                        className={`w-full aspect-square bg-${theme.colors.background.main} flex items-center justify-center`}
                      >
                        <FaMusic
                          className={`text-4xl text-${theme.colors.text.muted}`}
                        />
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center">
                      <div className="opacity-0 group-hover:opacity-100 transform scale-90 group-hover:scale-100 transition-all">
                        <button
                          className={`bg-${theme.colors.primary.main} text-white rounded-full p-3 shadow-lg`}
                        >
  ...
  return (
    <div className={`h-full flex flex-col bg-${theme.colors.background.main}`}>
        ...
        <div className="w-full lg:w-1/2 flex flex-col p-6">
          ...
          <div className="flex-1 flex items-center justify-center mb-6">
            <div className="relative max-w-md w-full aspect-square">
              <div
                className={`w-full h-full rounded-lg overflow-hidden shadow-2xl bg-${theme.colors.background.secondary}`}
              >
                {currentTrack?.imageUrl ? (
                  ...
                  <div
                    className={`w-full h-full flex items-center justify-center bg-gradient-to-br from-${theme.colors.primary.main} to-${theme.colors.primary.dark}`}
                  >
          ...
          <div className="text-center lg:text-left">
            <h1
              className={`text-3xl font-bold text-${theme.colors.text.primary} mb-2 truncate`}
            >
              {currentTrack.title}
            </h1>
            <h2
              className={`text-xl text-${theme.colors.text.muted} mb-4 truncate`}
            >
              {currentTrack.artist || "Unknown Artist"}
            </h2>
            <div
              className={`text-sm text-${theme.colors.text.muted} space-y-1`}
            >
              <p>
        ...
        <div className="w-full lg:w-1/2 flex flex-col border-l border-gray-700">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {showQueue ? (
                  ...
                  >
                    <div className="w-8 text-center text-sm text-gray-400 mr-3">
                      {index === currentTrackIndex && isPlaying ? (
                        <FaPlay
                          className={`text-${theme.colors.primary.main}`}
                        />
                      ) : (
                        index + 1
                      )}
                    ...
                    <div className="w-10 h-10 rounded mr-3 flex-shrink-0">
                      {track.imageUrl ? (
                        ...
                        <div
                          className={`w-full h-full bg-${theme.colors.background.main} rounded flex items-center justify-center`}
                        >
                          <FaMusic
                            className={`text-${theme.colors.text.muted} text-sm`}
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div
                      className={`text-sm text-${theme.colors.text.muted} ml-2`}
                    >
              ...
              /* Lyrics View */
              <div
                className={`bg-${theme.colors.background.secondary} rounded-lg p-6 h-full flex items-center justify-center`}
              >
                <div className="text-center">
                  <FaMusic
                    className={`text-4xl text-${theme.colors.text.muted} mb-4 mx-auto`}
                  />
      ...
      {/* Player Controls */}
      <div
        className={`border-t border-${theme.colors.border} bg-${theme.colors.background.secondary}`}
      >
        <Controls
          isPlaying={isPlaying}
