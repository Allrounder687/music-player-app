    ...
    <div className="p-6 h-full overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6">
        <div>
          ...
          <p
            className={`text-${theme.colors.text.muted} flex items-center gap-2`}
          >
            {isLoading ? (
              <>
              ...
              <>
                ...
                {selectedTracks.length > 0 && (
                  <span
                    className={`ml-2 px-2 py-1 bg-${theme.colors.primary.main} text-${theme.colors.text.primary} rounded-full text-sm`}
                  >
      ...
      {/* Search and Controls */}
      <div
        className={`bg-${theme.colors.background.secondary} rounded-lg p-4 mb-6 space-y-4`}
      >
        {/* Search Bar */}
        <div className="relative">
          <FaSearch
            className={`absolute left-3 top-1/2 transform -translate-y-1/2 text-${theme.colors.text.muted}`}
          />
          <input
            type="text"
            value={filter}
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-sm text-${theme.colors.text.muted}`}>
              Sort by:
            </span>
            <select
              value={sortBy}
      ...
      {tracks.length === 0 ? (
        <div className={`text-center py-16 text-${theme.colors.text.muted}`}>
          ...
          <p className="mb-4">
            Start by importing some music files to your library.
          </p>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
          {filteredAndSortedTracks.map((track) => (
            <div
              ...
              } transition-all duration-200 cursor-pointer group relative shadow-sm hover:shadow-md ${
                selectedTracks.includes(track.id)
                  ? `ring-2 ring-${theme.colors.primary.main}`
                  : ""
              }`}
            >
              {/* Selection Checkbox */}
              ...
              <div className="relative" onClick={() => playTrack(track)}>
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transform scale-90 group-hover:scale-100 transition-all">
                    <button
                      className={`bg-${theme.colors.primary.main} text-${theme.colors.text.primary} rounded-full p-3 shadow-lg hover:shadow-xl transition-shadow`}
                    >
                ...
                {currentTrack?.id === track.id && (
                  <div
                    className={`absolute top-2 right-2 bg-${theme.colors.primary.main} text-${theme.colors.text.primary} rounded-full p-2`}
                  >
              ...
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-semibold truncate text-${
                  ...
                  <button
                    ...
                    className={`ml-2 p-1 rounded-full hover:bg-${
                ...
                <div
                  className={`text-xs text-${theme.colors.text.muted} flex justify-between items-center`}
                >
                  <span
        ...
        /* List View */
        <div
          className={`bg-${theme.colors.background.secondary} rounded-lg overflow-hidden shadow-sm`}
        >
          {/* Table Header */}
          <div
            className={`grid grid-cols-12 gap-4 px-4 py-3 border-b border-${theme.colors.border} text-${theme.colors.text.muted} text-sm font-medium bg-${theme.colors.background.main}`}
          >
            <div className="col-span-1 flex items-center">
              <input
                type="checkbox"
                checked={
                  selectedTracks.length === filteredAndSortedTracks.length &&
                  filteredAndSortedTracks.length > 0
                }
                onChange={(e) => {
                  if (e.target.checked) {
          ...
          <div className="divide-y divide-gray-700">
            {filteredAndSortedTracks.map((track, index) => (
              <div
                key={track.id}
                className={`grid grid-cols-12 gap-4 px-4 py-3 hover:bg-${
                  theme.colors.background.tertiary
                  ...
                  selectedTracks.includes(track.id)
                    ? `bg-${theme.colors.primary.main} bg-opacity-10`
                    : ""
                }`}
                  ...
                  {!selectedTracks.includes(track.id) && (
                    <span
                      className={`text-${
                        ...
                        selectedTracks.includes(track.id) ? "hidden" : ""
                      }`}
                    >
                      {currentTrack?.id === track.id ? (
                        <FaPlay
                ...
                <div className="col-span-4 flex items-center min-w-0">
                  <div className="flex-shrink-0 w-10 h-10 mr-3">
                    {track.imageUrl ? (
                      ...
                      <div
                        className={`w-10 h-10 bg-${theme.colors.background.main} rounded flex items-center justify-center`}
                      >
                        <FaMusic
